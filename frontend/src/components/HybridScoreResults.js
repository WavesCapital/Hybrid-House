...
const HybridScoreResults = () => {
  ...
  const generateShareImage = useCallback(async () => {
    ...
    // Main score text without glow (clean white text)
    ctx.fillStyle = 'white';
    ctx.textAlign = 'center';
...
Think you can beat this? Get scored at HybridLab.io 🚀`;
      ...
      const fallbackText = `💥 Just scored ${hybridScoreValue}/100 on the Hybrid Athlete test! Think you can beat it? HybridLab.io 🚀`;
      if (navigator.share) {
        navigator.share({
          title: '🏆 My Hybrid Athlete Score',
          text: fallbackText
