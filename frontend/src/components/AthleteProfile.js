...
const AthleteProfile = () => {
  ...
  const generateShareImage = async () => {
    ...
    // Key metrics
    const bodyWeight = Math.round(data.scoreInputsUsed.bodyWeightKg * 2.20462);
    const mileTime = `${Math.floor(data.scoreInputsUsed.mileSeconds / 60)}:${String(Math.round(data.scoreInputsUsed.mileSeconds % 60)).padStart(2, '0')}`;
    const bench1RM = data.scoreInputsUsed.bench1RmKg ? Math.round(data.scoreInputsUsed.bench1RmKg * 2.20462) : null;
    ...
    let metricsY = 520;
    ctx.fillText(`Body Weight: ${bodyWeight} lbs`, 150, metricsY);
    ctx.fillText(`Mile Time: ${mileTime}`, 150, metricsY + 40);
    if (bench1RM) {
      ctx.fillText(`Bench Press: ${bench1RM} lbs`, 150, metricsY + 80);
    }
    
    ctx.fillText(`Body Fat: ${data.scoreInputsUsed.bodyFatPercent}%`, 500, metricsY);
    ctx.fillText(`VO2 Max: ${data.scoreInputsUsed.vo2Max}`, 500, metricsY + 40);
    ctx.fillText(`HRV: ${data.scoreInputsUsed.hrvMs} ms`, 500, metricsY + 80);
    
    // Branding
    ctx.fillStyle = '#4b5563';
    ctx.font = '16px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('Generated by Athlete Profile App', canvas.width / 2, canvas.height - 30);
  ...
  const handleShare = async () => {
    try {
      ...
      const shareText = `🏆 My Hybrid Athlete Score: ${Math.round(parseFloat(data.hybridScore))}/100 - Get yours at ${window.location.origin}`;
      if (navigator.share) {
        navigator.share({
          title: 'My Hybrid Athlete Score',
          text: shareText
