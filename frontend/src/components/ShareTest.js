...
const ShareTest = () => {
  ...
  const ComponentRenderer = ({ component }) => {
    ...
    return (
      <Rnd
        ...
        bounds="parent"
        className={`${selectedComponent?.id === component.id ? 'ring-2 ring-[#08F0FF]' : ''} cursor-move`}
  ...
  return (
    <div className="min-h-screen bg-[#0E0E11] text-white p-8">
      <h1 className="text-2xl font-bold mb-4">Share Component Test</h1>
      ...
      <div className="flex gap-8">
        ...
        <div className="w-64 space-y-4">
          ...
          <div className="mt-4">
            ...
            {components.map(comp => (
              <div key={comp.id} className="text-xs text-white/60">
                {comp.type} at ({comp.x}, {comp.y})
              </div>
            ))}
          </div>
        </div>
